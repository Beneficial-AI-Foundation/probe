...
- `schemas/atom-envelope.schema.json` rejected valid single-tool specs/proofs envelopes: the single-tool envelope accepted `*/specs` and `*/proofs` in its `schema` pattern but still validated `data` as a dictionary of full atoms, so entries like `{ "specified": true }` (P7) failed. Split the single-tool envelope into an atoms branch (`*/atoms`, `*/enriched-atoms`, `*/extract` → atom dictionary) and a generic branch (`*/specs`, `*/proofs`, `*/stubs`, `*/verification-report` → unconstrained dictionary), which also adds the KB-registered `probe-verus/stubs` and `probe-verus/verification-report` schemas that the pattern previously omitted.
