...
- Lean `def`s standing for Rust functions are implementations in every pipeline: a translation target inherits its exec's color, a specified `def` is colored by its primary-spec theorem's status, and an Aeneas-generated `def` (`rust-source`) without a spec is Yellow — so probe-lean-only extracts of Aeneas code color like the merged view
