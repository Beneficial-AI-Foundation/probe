...
- Color scheme reworked around one meaning per color, with role and language as separate axes; `scripts/count-colors.sh` implements it per-atom across all pipelines (incl. Lean and merged files) with four role groups broken down by language and a `--per-atom` mode for VeriLib. See `docs/verification-statuses.md` for the full scheme. (Translation `def`s inherit the Rust exec's verify-status to compensate for probe-aeneas#23.)
- Lean `def`s standing for Rust functions are implementations in every pipeline: a translation target inherits its exec's color, a `def` with a documented primary-spec (spec attribute or `<def>_spec` naming; probe-lean's sole-spec inference is ignored as too loose) is colored by that theorem's status, and an Aeneas-generated `def` (`rust-source`) without a spec is Yellow — so probe-lean-only extracts of Aeneas code color like the merged view
