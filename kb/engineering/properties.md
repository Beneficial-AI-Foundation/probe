...
For Aeneas projects, a Rust function is **out of verification scope** — `is-disabled: true`, no `verification-status` — exactly when it is not compiled into the verified library in the Aeneas build, or its Lean translation is explicitly annotated out of scope. Formally: `is-disabled: true ⟺ cfg-inactive ∨ non-library-target ∨ translation carries @[out_of_scope]`:
...
3. **`@[out_of_scope]`** — the generated Lean translation carries an out-of-scope attribute, declaring "this translation will not be verified". This attribute is the explicit opt-out.
